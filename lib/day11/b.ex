defmodule Day11.B do
  ...
  def call do
    input = File.read!("lib/day11/input_example") |> trim()
  ...
  def parse_monkey(input) do
    ...
    divisible_by = to_integer(divisible_by)
    [[_, true_monkey]] = Regex.scan(~r/If true: throw to monkey (.+)/, input)
    true_monkey = to_integer(true_monkey)
    [[_, false_monkey]] = Regex.scan(~r/If false: throw to monkey (.+)/, input)
    false_monkey = to_integer(false_monkey)

    test = fn value ->
      if rem(value, divisible_by) == 0 do
        true_monkey
      else
        false_monkey
      end
  ...
  def inspect_items(data, 10_000), do: data

  def inspect_items(data, round) do
    puts("performing round ##{round}...")
